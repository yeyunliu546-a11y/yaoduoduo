					...
					<block v-if="itemStyle.column === 1">
						<view class="dis-flex">
							...
							<view class="goods-item_right">
								...
								<view class="goods-tags" style="display: flex; flex-wrap: wrap; gap: 4px; margin: 4px 0;">
								    <text v-if="dataItem.manufacturer" style="font-size: 20rpx; color: #666; background: #f5f5f5; padding: 2rpx 8rpx; border-radius: 4rpx;">{{ dataItem.manufacturer }}</text>
								    <text v-if="dataItem.standard" style="font-size: 20rpx; color: #007aff; background: #eaf2ff; padding: 2rpx 8rpx; border-radius: 4rpx;">{{ dataItem.standard }}</text>
								</view>
	...
	export default {
		...
		created() {
			...
			if (this.paramsData.source == "auto") {
				...
				// --- 核心修改区域 Start ---
				this.queryParams = {
					// 1. 基础分页 (文档要求小写 page, limit)
					page: 1,
					limit: autoData.showNum,
					
					// 2. 排序字段 (文档要求 sortType, 原代码是 SortCol)
					sortType: autoData.goodsSort, 
					
					// 3. 搜索与筛选 (对应文档中的可选参数)
					// 如果 autoData 里没有这些字段，默认传空字符串，以免后端报错
					key: autoData.searchKey || '',           // 关键词
					manufacturer: autoData.manufacturer || '', // 厂家
					packageType: autoData.packageType || '',   // 包装类型
					standard: autoData.standard || '',         // 标准类型
					
					// 4. 分类 (保留原逻辑，假设后端支持此字段，或自行根据文档调整)
					// 注意：文档中没有明确写 category 参数，如果接口不支持，可能需要去掉或改为 key 搜索
